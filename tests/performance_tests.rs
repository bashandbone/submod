...
mod tests {
    ...
    fn test_multiple_submodules_performance() {
        ...
        for i in 0..10 {
            let remote = harness.create_test_remote(&format!("perf_repo_{}", i))
        ...
        harness.run_submod_success(&["check"]).expect("Failed to run check");
        let check_duration = check_start.elapsed();
        println!("Check time for 10 submodules: {:?}", check_duration);

        // Test update performance
        let update_start = Instant::now();
        harness.run_submod_success(&["update"]).expect("Failed to run update");
        let update_duration = update_start.elapsed();
        println!("Update time for 10 submodules: {:?}", update_duration);

        // Performance assertions (these are rough guidelines)
        assert!(add_duration.as_secs() < 30, "Adding 10 submodules took too long: {:?}", add_duration);
        assert!(check_duration.as_secs() < 5, "Checking 10 submodules took too long: {:?}", check_duration);
        assert!(update_duration.as_secs() < 20, "Updating 10 submodules took too long: {:?}", update_duration);
    }
    ...
    fn test_large_config_file_performance() {
        ...
        let mut large_config = String::from(r#"[defaults]
ignore = "dirty"
update = "checkout"
...
"#,
        ...
        harness.create_config(&large_config).expect("Failed to create large config");
        ...
        assert!(config_create_duration.as_millis() < 1000, "Config creation too slow: {:?}", config_create_duration);
        assert!(parse_duration.as_secs() < 2, "Config parsing too slow: {:?}", parse_duration);
    }
    ...
    fn test_deep_directory_structure() {
        ...
        let remote_repo = harness.create_test_remote("deep_structure").expect("Failed to create remote");
        let remote_url = format!("file://{}", remote_repo.display());
        ...
        for deep_path in &deep_paths {
            ...
            assert!(harness.file_exists(&format!("{}/.git", deep_path)));
        }

        // Performance assertion
        assert!(duration.as_secs() < 15, "Deep directory creation too slow: {:?}", duration);
    }
    ...
    fn test_sparse_checkout_with_many_patterns() {
        ...
        let remote_repo = harness.create_complex_remote("many_patterns").expect("Failed to create remote");
        let remote_url = format!("file://{}", remote_repo.display());
        ...
        let sparse_file = harness.work_dir.join("lib/many-patterns/.git/info/sparse-checkout");
        ...
        // Performance assertion
        assert!(duration.as_secs() < 10, "Many patterns processing too slow: {:?}", duration);
    }
    ...
    fn test_config_serialization_performance() {
        ...
        let mut remote_repos = Vec::new();
        for i in 0..20 {
            let remote = harness.create_test_remote(&format!("serial_repo_{}", i))
                .expect("Failed to create remote");
            remote_repos.push(remote);
        }
        ...
        for (i, remote_repo) in remote_repos.iter().enumerate() {
            ...
            println!("Submodule {} add time: {:?}", i, duration);
        }
        ...
        let submodule_count = final_config.lines()
            .filter(|line| line.starts_with("[serial-"))
            .count();
        assert_eq!(submodule_count, 20);

        // Performance assertion
        assert!(total_duration.as_secs() < 60, "Config serialization too slow: {:?}", total_duration);
    }
    ...
    fn test_concurrent_check_operations() {
        ...
        for i in 0..5 {
            let remote_repo = harness.create_test_remote(&format!("concurrent_{}", i))
                .expect("Failed to create remote");
            let remote_url = format!("file://{}", remote_repo.display());
        ...
        // Performance assertion
        assert!(duration.as_secs() < 30, "Consecutive checks too slow: {:?}", duration);
    }
    ...
    fn test_memory_usage_with_large_operations() {
        ...
        for i in 0..25 {
            let remote_repo = harness.create_test_remote(&format!("memory_test_{}", i))
                .expect("Failed to create remote");
            let remote_url = format!("file://{}", remote_repo.display());
            ...
            harness.run_submod_success(&[
                ...
                &format!("lib/memory{}", i),
                &remote_url,
                "--sparse-paths", "src,docs,include,tests,examples",
            ]).expect("Failed to add submodule");
        }
        ...
        assert!(duration.as_secs() < 120, "Large operations too slow: {:?}", duration);
    }
    ...
    fn test_file_system_performance() {
        ...
        let remote_repo = harness.create_complex_remote("fs_perf").expect("Failed to create remote");
        let remote_url = format!("file://{}", remote_repo.display());
        ...
        harness.run_submod_success(&[
            ...
            "lib/fs-perf",
            &remote_url,
            "--sparse-paths", "src,docs,tests,examples",
        ]).expect("Failed to add submodule");
        ...
        // Performance assertion
        assert!(duration.as_secs() < 20, "FS operations too slow: {:?}", duration);
    }
...
"#;
        ...
        let start_time = Instant::now();
        harness.create_config(unicode_config).expect("Failed to create unicode config");
        ...
        // Performance assertion
        assert!(duration.as_millis() < 2000, "Unicode processing too slow: {:?}", duration);
    }
}

...
impl TestHarness {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // `fscache`/`preloadindex` are in-memory read accelerators for the
        // thousands of fixture commands (matters most on Windows); the
        // untracked-cache and background helpers stay off so snapshots never
        // race index rewrites or helper side effects.

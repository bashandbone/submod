...
pub struct Cli {
    ...
    #[arg(short = "c", long = "config", global = true, default_value = "submod.toml", value_parser = clap::value_parser!(PathBuf), value_hint = clap::ValueHint::FilePath, about = "Path to the configuration file. Defaults to 'submod.toml' in the current directory.")]
    pub config: PathBuf,
}
...
pub enum Commands {
    ...
    Add {
        ...
        #[arg(short = "n", long = "name", value_parser = clap::value_parser!(String), hide = true)]
        name: Option<String>,
    ...
    Change {
        #[arg(required = true, value_parser = clap::value_parser!(String), value_hint = clap::ValueHint::CommandName, about = "The name of the submodule to change. Must match an existing submodule.")]
        name: String,

        #[arg(short = "p", long = "path", value_parser = clap::value_parser!(OsString), value_hint = clap::ValueHint::DirPath, about = "New local path for the submodule. Implies `nuke-it-from-orbit` if the path changes.")]
        path: Option<OsString>,

        #[arg(short = "b", long = "branch", value_parser(SerializableBranch::from_str), about = "Change the submodule's branch.")]
        branch: Option<String>,

        #[arg(short = "x", long = "sparse-paths", value_delimiter = ',', about = "Replace the sparse checkout paths (comma-separated), or add if not set. Use `--append` to append to existing sparse paths.", default_missing_value = "none")]
        sparse_paths: Option<Vec<String>>,
    ...
    #[subcommand(name = "disable", visible_alias = "d", help_heading = "Disable a Submodule", about = "Disables a submodule by name; sets its active status to false.")]
    Disable,
    ...
    /// Completely resets submodules -- deleting all associated files and records, reinitiating from your submod.toml. Ridley would be proud.
...
pub fn set_path(path: OsString) -> Result<String, anyhow::Error> {
    match path.to_str() {
        Some(utf8_path) => {
            Ok(utf8_path.to_string())
        }
        None => {
            // Could base64 encode or use lossy conversion
